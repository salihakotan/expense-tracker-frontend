...
const recordsReducer = (state:RecordState=defaultState,action:RecordAction):RecordState => {
    switch(action.type){
                ...
                case "UPDATE_RECORDS_ERROR":
                    ...
                    case "DELETE_RECORDS_START":
                        ...
                        case "DELETE_RECORDS_SUCCESS":
                            return {...state,loading:false,error:"",data:state.data.filter(record => record.id !== action.payload)};
